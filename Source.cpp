...
int main() {
	auto timer = Timer();
	try {
		std::fstream input(R"(..\LZW15\enwik9)", std::ios_base::in | std::ios_base::binary);
		if (!input.is_open())
			exit(1);
		auto output = stl::OpenOutputBitFile(R"(..\LZW15\testFile2.txt)");
		std::cout << "compression started....\n";
		timer.Start();
		compressFile(input, output);
		timer.Stop();
		printf("\nFile compression complete\n");
		printf("LZW15 coding compression time = %f seconds\n\n", timer.time());
		stl::closeOutputBitFile(output);

		auto input1 = stl::OpenInputBitFile(R"(..\LZW15\testFile2.txt)");
		std::fstream output1(R"(..\LZW15\testFile3.txt)", std::ios_base::out | std::ios_base::binary);
		printf("Expansion started....\n");
		timer.Start();
		expandFile(input1, output1);
		timer.Stop();
		printf("\nFile expansion complete\n");
		printf("LZW15 coding expansion time = %f seconds\n\n", timer.time());
		stl::closeInputBitFile(input1);
		output1.close();

		//print file sizes
		std::cout << std::format("Original file size = {} bytes\n", fileSize(fs::path(R"(..\LZW15\enwik9)")));
		std::cout << std::format("Compressed file size = {} bytes\n", fileSize(fs::path(R"(..\LZW15\testFile2.txt)")));
		std::cout << std::format("Expanded file size = {} bytes\n", fileSize(fs::path(R"(..\LZW15\testFile3.txt)")));
	}
